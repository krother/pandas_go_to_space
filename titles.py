...
the pandas were the most
magnificient space travelers.

They would seek new worlds
new tastes of bamboo and
boldly go where no one
had gone before.

Their ships would produce
plenty of data, so pandas
were also known as skillful
data scientists.
""".split('\n')

def create_text_bitmap(color=YELLOW, line_spacing=50):
    """
    ...
    for y, line in enumerate(text):
        cv2.putText(
            ...
            cv2.FONT_HERSHEY_DUPLEX, # FONT_HERSHEY_TRIPLEX,
            fontScale=1.5,
            color=color,
            thickness=3
        )
...
while True:
    ...
    rgb = cv2.cvtColor(frame[200:, 100:-100], cv2.COLOR_BGR2RGB)
    frames.append(rgb)

    #time.sleep(0.03)

cv2.destroyAllWindows()
imageio.mimsave('main_titles.gif', frames, fps=20)